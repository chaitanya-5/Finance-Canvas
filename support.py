...
piecolor= ['#F8F0C4','#fffbff', '#A9A9A9', '#ACD0F4']
...
barcolor= ['#F8F0C4','#fffbff', '#A9A9A9', '#ACD0F4']
...
def meraPie(df=None, names=None, values=None, color=None, width=None, height=None, hole=None, hole_text=None,
    ...
    fig = px.pie(data_frame=df, names=names, values=values, color=color, color_discrete_sequence=barcolor, hole=hole, width=width, height=height)
...
def meraLine(df=None, x=None, y=None, color=None, slider=True, title=None, height=180, width=None, show_legend=True):
    # Line Chart
    line = px.line(data_frame=df, x=x, y=y, color=color, color_discrete_sequence=barcolor, template="plotly_dark", height=height, width=width)
...
def meraScatter(df=None, x=None, y=None, color=None, size=None, slider=True, title=None, height=180, width=None,
                legend=False):
    scatter = px.scatter(data_frame=df, x=x, y=y, color=color, color_discrete_sequence=barcolor, size=size, template="plotly_dark", height=height,
                         width=width)
...
def meraHeatmap(df=None, x=None, y=None, text_auto=True, aspect='auto', height=None, width=None, title=None, color_scale='gray'):
    fig = px.imshow(pd.crosstab(df[x], df[y]), text_auto=text_auto, aspect=aspect, height=height, width=width,
                    template='plotly_dark',color_continuous_scale=color_scale)
    fig.update(layout_showlegend=False)
...
def month_bar(df=None, height=None, width=None):
    ...
    fig = px.bar(t, x='Month', y='Amount($)', color='Expense', color_discrete_sequence=barcolor, text_auto=True, height=height, width=width,
...
'''def month_bar(df=None, height=None, width=None):
    ...
    # Filter out datetime columns before aggregation

    numeric_columns = [col for col in df.columns if pd.api.types.is_numeric_dtype(df[col])]



    # Perform grouping and sum aggregation on numeric columns

    if 'Month' in df.columns and 'Expense' in df.columns and 'Amount(₹)' in df.columns:

        t = df.groupby(['Month', 'Expense'])[numeric_columns].sum().reset_index()

    else:

        raise ValueError("Required columns 'Month', 'Expense', 'Amount(₹)' not found in DataFrame.")
    ...
    if 'Month' in t.columns:
        ...
        t['Month'] = t['Month'].map(month_names)



    # Create bar chart using Plotly Express

    fig = px.bar(t, x='Month', y='Amount(₹)', color='Expense', text='Amount(₹)',

                 labels={'Month': 'Month', 'Amount(₹)': 'Amount (₹)', 'Expense': 'Expense'},
...
def meraSunburst(df=None, height=None, width=None):
    fig = px.sunburst(df, path=['Year', 'Expense', 'Note'], values='Amount($)', color_discrete_sequence=barcolor, height=height, width=width)
