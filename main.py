...
def delete_expense():
    if 'user_id' in session:
        user_id = session['user_id']
        if request.method == 'POST':
            expense_id = request.form.get('expense_id')  # Get the expense ID from the form

            try:
                # Construct the SQL query to delete the expense
                query = "DELETE FROM user_expenses WHERE user_id = {} AND id = {}".format(user_id, expense_id)
                print("SQL Query:", query)  # Print the SQL query for debugging
                print("expenseid", expense_id)
                # Execute the SQL query using the support module
                result = support.execute_query('delete', query)
                print("Execution Result:", result)  # Print the result of query execution

                if result:
                    flash("Expense deleted successfully.")
                else:
                    flash("No expense found with the specified ID.")
   ...
   if 'user_id' in session:
       ...
       if request.method == 'POST':
          ...
          try:
                ...
                # Call the edit_expense function to update the expense record

                support.execute_query('update', f"UPDATE user_expenses SET  expense='{new_type}', amount='{new_amount}' WHERE user_id={user_id} AND id={expense_id}")
